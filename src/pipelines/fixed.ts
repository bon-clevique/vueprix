...
// reference 価格が現価から極端に乖離している場合の上限 (誇大広告防止)。
// Amazon UI 上の割引率は通常 80% 程度が上限で、それを超える値は seller 設定の異常値か
// Notion 手動入力ミスを疑う。本値超過時は Notion 手動 reference を採用せず Keepa fallback に落とす。
...
// publishFixedCandidates 内で Notion manual reference を加味した上で再評価して閾値判定する。
...
// `collectFixed` で取れた Keepa fallback の reference と、Notion 手動入力 参考定価
// を比較して最終 reference を決める。優先順位:
//   1. Notion 手動入力 参考定価 (bon が運用で設定する希望小売価格 / 旧定価)
//   2. Keepa fallback (referencePrice/dropPercent)
// 採用判定 (各候補共通):
...
export const resolveFixedReference = (
  ...
  dropPercent: number;
  referenceSource: 'manual-reference-price' | 'keepa';
} => {
