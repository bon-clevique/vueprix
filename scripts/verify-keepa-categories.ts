/**
 * Verify Keepa category IDs configured in src/config.ts.
 *
 * Usage: tsx scripts/verify-keepa-categories.ts
 *
 * Reads KEEPA_API_KEY from .env and queries the /category endpoint for each
 * configured category. Prints the official name and ancestor chain, plus
 * remaining tokens.
 *
...
const main = async (): Promise<void> => {
  ...
  console.log(`Verifying ${KEEPA_CATEGORIES.length} categories on Keepa domain ${KEEPA_DOMAIN} (Amazon.co.jp)\n`);
  for (const id of KEEPA_CATEGORIES) {
    try {
